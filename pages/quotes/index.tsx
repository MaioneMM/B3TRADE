...
export async function getStaticProps() {
  ...
  try {
    ...
    actualData = response.data.data.map(
      (stock: any) => {
        return {
          logoid: stock.d[0],
          name: stock.d[1],
    ...
    console.error('TradingView scan failed:', error);
  }
...
const Quotes = ({ stocks }: InferGetStaticPropsType<typeof getStaticProps>) => {
  return (
    <div>
      <Head>
        <title>B3Trade - Cotação das Ações da Bovespa</title>
      </Head>
      <Layout>
        <Header />
